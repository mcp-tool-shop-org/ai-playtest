...
const GENERATOR_NAME = '@mcptoolshop/ai-playtest';

function generatorMeta(): { name: string; version: string; reportFormat: number; schemaVersion: number } {
  return { name: GENERATOR_NAME, version: VERSION, reportFormat: REPORT_FORMAT, schemaVersion: SCHEMA_VERSION };
}
...
export async function readRun(runDir: string): Promise<RunContents> {
  ...
  for (const e of entries.sort()) {
    ...
    // Seat meta.json is runner-core. Honor a stamp if present; do not require one.
    assertReadableFormat(join(dir, 'meta.json'), stampOf(meta));
...
function generatorStamp(): string {
  return `_Generated by ${GENERATOR_NAME} ${VERSION} (schemaVersion ${SCHEMA_VERSION}, report format ${REPORT_FORMAT})._`;
}

function stampOf(raw: unknown): { reportFormat?: number; schemaVersion?: number } {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return {};
  const rec = raw as Record<string, unknown>;
  const gen = rec.generator && typeof rec.generator === 'object' && !Array.isArray(rec.generator)
    ? rec.generator as Record<string, unknown>
    : undefined;
  const reportFormat = typeof rec.reportFormat === 'number' ? rec.reportFormat
    : typeof gen?.reportFormat === 'number' ? gen.reportFormat
    : undefined;
  const schemaVersion = typeof rec.schemaVersion === 'number' ? rec.schemaVersion
    : typeof gen?.schemaVersion === 'number' ? gen.schemaVersion
    : undefined;
  return { reportFormat, schemaVersion };
}

/** Refuse to rebuild a run written for a newer report format than this tool emits. */
function assertReadableFormat(where: string, stamp: { reportFormat?: number }): void {
  if (typeof stamp.reportFormat === 'number' && stamp.reportFormat > REPORT_FORMAT) {
    throw new ReportError(
      `${where} is report format ${stamp.reportFormat}; this tool writes format ${REPORT_FORMAT}`,
...
function verdictOf(s: SeatSummary): boolean | null {
  const p = scoringPanel(s);
  if (p) return p.alive;
...
export function renderReport(name: string, label: string, seats: SeatSummary[], expectedCriteria?: string[], skipped: SkippedSeatDir[] = []): string {
  ...
  lines.push('');
  lines.push('`!` = that seat\'s jurors split; `met` is seats (yes/no/unanswered); `agreement` is jurors on a 1-seat run and seats otherwise.');
  lines.push('');
  ...
  if (seats.some((s) => s.coverage)) {
    ...
    lines.push('');
    lines.push('`repeat` = consecutive same input; `loop` = 2-gram cycle; `H(a)` = action entropy in bits; bold is thin/moderate/broad.');
    lines.push('');
...
export type SingleRunReport = {
  kind: 'single-run-report';
  name: string;
  label: string;
  schemaVersion: number;
  generator: { name: string; version: string; reportFormat: number; schemaVersion: number };
  seats: SeatSummary[];
  alive: number;
  wouldPlayAgain: number;
  asked: number;
  skipped: SkippedSeatDir[];
};
...
export function renderReportJson(
  ...
  return {
    ...
    seats: [...seats],
    alive: seats.filter((s) => verdictOf(s) === true).length,
    wouldPlayAgain: seats.filter(wouldPlayAgainOf).length,
    asked: seats.length + skipped.length,
    skipped: [...skipped],
  };
}

async function assertExistingSidecarReadable(runDir: string): Promise<void> {
  const sidecarPath = join(runDir, 'REPORT.json');
  const existing = await readOptional(sidecarPath);
  if (!existing) return;
  try {
    assertReadableFormat(sidecarPath, stampOf(JSON.parse(existing)));
  } catch (err) {
    if (err instanceof ReportError) throw err;
    // Corrupt sidecar is overwritten by the rebuild, same as a missing one.
  }
}
...
export async function writeReport(name: string, runDir: string, label: string, expectedCriteria?: string[]): Promise<string> {
  ...
  await assertExistingSidecarReadable(runDir);
  ...
  const payload = renderReportJson(name, label, seats, expectedCriteria, seats.skipped);
  await writeFile(join(runDir, 'REPORT.json'), JSON.stringify(payload, null, 2) + '\n', 'utf8');
  return path;
}
...
export async function writeAggregateReport(
  ...
  await assertExistingSidecarReadable(dir);
